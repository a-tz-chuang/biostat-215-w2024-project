...
ADD AFT variable selection here
...
# add method because calPlot depends on calling predictSurvProb method
...
myCalPlotSettings(fit.exp, "Exponential AFT Prediction")
```
```{r}
myCalPlotSettings(fit.wei, "Weibull AFT Prediction")
```


```{r}
myCalPlotSettings(fit.lognormal, "Log-normal AFT Prediction")
```
...
```
```{r}
myCalPlotSettings(lasso_temp, "LASSO", formula = full_model_formula)
```
