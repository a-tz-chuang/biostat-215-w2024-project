...
```{r}
rot.cont <- rotterdam %>% select(age, grade, nodes, pgr, er, rtime, dtime) 
rot.cat <- rotterdam %>% select(year, meno, size, hormon, chemo, recur, death)
  ...
  lapply(
    function(.x) {
      tbl_summary(
        ...
        include = c(age, grade, nodes, pgr, er, rtime, dtime),
        missing = "no",
...
### Weibull AFT with Relapse Indicator

Weibull AFT model forward selection via BIC:

```{r}
full_formula <- Surv(dtime, death) ~ age + meno + size + 
  grade + nodes + pgr + er + hormon + chemo +
  dtime:recur + recur
relapse_weiAFT <- stepAIC(survreg(full_formula,
                               data = rotterdam_train, dist = "weibull",
                               x = TRUE, y = TRUE),
                       full_formula,
                       direction = "backward", k = log(n))
pam.survreg(relapse_weiAFT, x.mat = rotterdam_train)
```
...
```{r}
print("Uno's Corrected C Statistics")
print("Cox Proportional Hazards Stepwise Forward Selection via AIC")
concordance(rot_sfAIC, timewt = "n/G2", newdata = rotterdam_test)$concordance
print("Cox Proportional Hazards Stepwise Forward Selection via BIC")
concordance(rot_sfBIC, timewt = "n/G2", newdata = rotterdam_test)$concordance
print("Cox Proportional Hazards LASSO Estimate (10-fold CV)")
concordance(lasso_temp, timewt = "n/G2", newdata = rotterdam_test)$concordance
print("Weibull AFT (Relapse Included), Stepwise Backward via BIC")
concordance(relapse_weiAFT, timewt = "n/G2", newdata = rotterdam_test)$concordance
print("Harrel's C Statistics")
print("Cox MH (Time-dependent Covariates), Stepwise Backward via BIC")
concordance(rott_sbBIC, newdata = rottest_t)$concordance
print("Cox MH (Time-dependent Covariates), Stepwise Forward via AIC")
